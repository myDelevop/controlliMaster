...
import it.bz.prov.controlli.security.authorization.OppabLdapAuthorizationDeniedException;
...
public class OppabLdapAuthenticationProvider implements AuthenticationProvider {
	...
	private static final Log _logger = LogFactory.getLog(OppabLdapAuthenticationProvider.class);
    ...
    /**
     * Create an instance for bean call
     */
    public OppabLdapAuthenticationProvider(){
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        _oppabLogger.info(this.getClass().getSimpleName() + ": Inizio autenticazione...");
        ...
        _oppabLogger.info(this.getClass().getSimpleName() + ": Username (senza domain): " + username);

        if (!StringUtils.hasLength(username)) {
        	_oppabLogger.error(this.getClass().getSimpleName() + ": Lo username � vuoto");
        ...
        if (password.length() == 0) {
        	_oppabLogger.error(this.getClass().getSimpleName() + ": La password � vuota");
        ...
        try {
            ...
            _oppabLogger.info(this.getClass().getSimpleName() + ": Utente trovato: " + userData.getNameInNamespace());
            ...
            if(extraAuthorities.size()==0){
            	_oppabLogger.warn(this.getClass().getSimpleName() + ": Non sono stati trovati ruoli per questo utente. Non si dispone delle autorizzazioni necessarie per poter utilizzare questa applicazione");
            ...
            _oppabLogger.info(this.getClass().getSimpleName() + ": Termine autenticazione...");

            return createSuccessfulAuthentication(userToken, user);

        } catch (NamingException ldapAccessFailure) {
        	_oppabLogger.error("Errore durante la fase di autenticazione: " + ldapAccessFailure.getMessage());
